...
object ExtractCWURL {
  ...
  def main(args: Array[String]) {
    ...
    val conf: SparkConf = new SparkConf()
      .setAppName("Extract url from clueweb: " + cwyear + " " + task)
        ...
        pathQid => {
          ...
          sc
            ...
            .map { kv =>
              ...
              (cwdoc.getDocid, cwdoc.getHeaderMetadataItem("WARC-Target-URI"))
            }
            .filter(v => v._2 != null)
            .map{case(cwid, url)=> cwid + " " + url}
            .coalesce(1)
            .saveAsTextFile(outputdir + "/" + qid)
          System.out.println("Finished:" + year +  "_"  + qid)
        }
